<template>
    ...
    <div class="text-center">
      <div class="bg-white p-6 md:p-4 rounded-lg shadow-md max-w-md mx-auto">
        ...
        <p class="text-gray-600 mb-6">
          Mahsulotdagi
          <UIcon name="material-symbols:favorite-outline-rounded" /> ni bosing , Akkauntingizga kiring va sevimliga
          saqlagan barcha mahsulotingiz saqlanib qoladi
        </p>
        <button @click="store.registerModal = !store.registerModal" v-if="!store.profileData" 
  ...
  <div v-if="favoriteProducts.length" class="container">
    <h2 class="text-2xl font-bold my-6">Sevimli mahsulotlar soni: {{
      store.profileData.data?.favorites?.favoriteProducts.length }}</h2>
  </div>
  <div v-if="favoriteProducts.length" class="container grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">
    <div @click="navigaItem(product.product.id, product.product.name)" v-for="product in favoriteProducts"
      ...
      <div class="flex justify-center items-center aspect-square mb-4">
        <img v-if="product.product.productImages?.length > 0"
          :src="`https://api.albaraka.uz/${product.product.productImages[0].imageEntity.localImagePath}`"
      ...
      <div class="flex flex-col justify-between flex-grow">
        ...
        <div class="flex items-center justify-between mt-auto">
          ...
          <img v-if="product.product.marketResultmodel?.url" class="w-16"
            :src="getMarketLogo(product.product.marketResultmodel.url)" alt="Market Logo" />
        </div>
      </div>
    </div>
  </div>
