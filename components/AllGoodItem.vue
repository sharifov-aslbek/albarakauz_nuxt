  ...
  <div class="container py-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        " class="flex items-center gap-4 my-4">
          <n-rate readonly :default-value="getParsedProductModel(store.product.productModel).Rating" />
    ...
    <div class="flex flex-col md:flex-row pb-5 justify-between gap-5 w-full max-w-full mr-7 py-12">
      <div class="flex w-full h-[500px]">
        <n-scrollbar class="sm:block hidden" style="max-height: 500px; width: 80px;">
        ...
        <div class="sm:w-full sm:max-w-[400px] w-full">
          ...
          <UCarousel v-if="store.product.productImages && store.product.productImages.length > 0" ref="carousel"
            ...
            <n-image v-if="item.imageEntity && item.imageEntity.localImagePath"
              class="w-full sm:w-[420px] rounded-lg object-cover"
      ...
      <div class="flex w-full max-w-[450px] flex-col items-start space-y-6">
        ...
        <p class="my-5" v-if="!getParsedProductModel(store.product.productModel)?.main_features?.data"
          v-html="getParsedProductModel(store.product.productModel).short_description_ru"></p>


        <div v-if="store.product.marketResultmodel.name === 'Allgood' &&
          ...
          <div
            ...
            :key="key" class="flex justify-between text-xs md:text-sm text-gray-700">
   ...
   class="flex flex-col items-center justify-center border border-gray-300 shadow-xl px-3 rounded-lg h-full py-6">
    ...
    <div id="table" class="flex flex-col xl:flex-row gap-6 justify-center">
      <n-tabs type="line" animated class="w-full xl:w-[70%]" :style="tabStyle">
        <template #tabs="{ panes }">
          <div class="flex w-full flex-wrap">
            <div v-for="pane in panes" :key="pane.name"
              class="flex-1 text-center border border-gray-300 py-2 cursor-pointer text-xs sm:text-sm md:text-base"
        ...
        <n-tab-pane name="oasis" tab="Описание">
          <p v-if="store.product.description" class="w-full text-sm md:text-base" v-html="store.product.description">
          </p>
          ...
          <div v-if="store.product.descriptionUz || store.product.descriptionRu"
            ...
            <p v-if="store.product.descriptionUz" class="w-full mb-6 md:mb-12 text-sm md:text-base">
              {{ store.product.descriptionUz }}
            </p>
            <p v-else class="w-full mb-6 md:mb-12 text-sm md:text-base">
              {{ store.product.descriptionRu }}
            </p>
          </div>
        </n-tab-pane>

        <n-tab-pane v-if="getParsedProductModel(store.product.productModel)?.uz?.characteristics" name="xarakteristika"
          tab="Характеристика">
          <div
            v-for="([key, value], index) in Object.entries(getParsedProductModel(store.product.productModel)?.uz.characteristics)"
            :key="key" class="flex justify-between text-xs sm:text-sm md:text-base text-gray-700 w-full gap-3">
            <span class="text-gray-400 py-2 truncate">
              {{ key }}
            </span>
            <span class="flex-1 border-b border-dotted border-gray-300 mx-2"></span>
      ...
      <!-- Olcha bo'lsa qo'shimcha ma'lumot -->
      <div class="w-full xl:w-[28%]">
        <div v-if="store.product.marketResultmodel.name == 'Olcha'"
          class="flex flex-col justify-between border-gray-300 border gap-6 sm:gap-8 rounded-lg py-4 sm:py-8 px-4">
          <ItemOlchaMarketMap />

          <h3 v-if="store.product.marketResultmodel.name === 'Olcha' &&
            getParsedProductModel(store.product.productModel)?.store">
            <span class="flex items-center font-bold text-sm sm:text-lg gap-2 mb-2">
              <UIcon class="size-6 sm:size-8" name="material-symbols-light:home-work-rounded" />
              {{ getParsedProductModel(store.product.productModel).store.name_ru }}
            </span>
            <p class="text-xs sm:text-sm md:text-base">
              {{ getParsedProductModel(store.product.productModel).store.address_ru }}
            </p>

            <h3 class="flex text-sm sm:text-base md:text-lg font-semibold gap-2 mt-4 sm:mt-7 items-center">
              <UIcon class="size-6 sm:size-8" name="material-symbols:delivery-truck-bolt-outline-rounded" />
            ...
            <h3 class="flex text-sm sm:text-base md:text-lg font-semibold gap-2 items-center mt-4 sm:mt-8">
              <UIcon class="size-6 sm:size-8" name="ph:hand-coins-fill" />
              Рассрочка от:
            </h3>
            <h3 v-if="
            ...
            " class="text-brand-green text-sm sm:text-lg font-semibold mt-2">
              {{
